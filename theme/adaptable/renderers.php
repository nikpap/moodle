...
EOT;
    ...
    public function get_footer_blocks($layoutrow = 'footerlayoutrow') {
        ...
        if ($PAGE->theme->settings->showtopfooterblock) {
            $output .= '<div class="container2 blockplace1"><div class="container">';
    ...
    public function footer() {
        ...
        $performanceinfo = '';
        if ($CFG->adminonlyperfinfo and is_siteadmin()) {
        ...
        if (!empty($PAGE->theme->settings->logo)) {
            ...
            if ($COURSE->id > 1) {
                ...
                switch ($PAGE->theme->settings->enableheading) {
                    case 'fullname':
                        // Full Course Name.
                        $retval .= '<span id="coursetitle">' . format_string($COURSE->fullname) . '</span>';
                        break;

                    case 'shortname':
                        // Short Course Name.
                        $retval .= '<span id="coursetitle">' . format_string($COURSE->shortname) . '</span>';
                        break;
        ...
        if ($display == 'default') {
            ...
            if ($COURSE->id > 1) {
                ...
                switch ($PAGE->theme->settings->enableheading) {
                    case 'fullname':
                        // Full Course Name.
                        $retval .= '<div id="sitetitle">' . $SITE->shortname;
                        $retval .= '<span id="coursetitle">' . format_string($COURSE->fullname) . '</span></div>';
                        break;

                    case 'shortname':
                        // Short Course Name.
                        $retval .= '<div id="sitetitle">' . $SITE->shortname;
                        $retval .= '<span id="coursetitle">' . format_string($COURSE->shortname) . '</span></div>';
                        break;
